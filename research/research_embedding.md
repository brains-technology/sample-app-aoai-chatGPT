...
Azure OpenAI のテキスト検索精度とリソース効率を最適化するために、同一条件下で `text-embedding-3-small` と `text-embedding-3-large` の性能を比較し、検索結果の一貫性・ストレージ使用量・表記ゆれへの強さなどの観点から、それぞれのモデルがもつ実用上の違いを明らかにしました。</br>
...
## 結論と考察
- 今回の条件・用途においては small モデルでも large モデルとほとんど差が出ず、コスト・速度面から考えると small モデルが推奨される
